...
description: "正文可写性检查。用于在准备开始小说正式正文、询问‘现在能不能写正文/可以开第一章了吗/这本书达到 Manuscript Ready 了吗’，或用户直接要求开始某章正文时，先判断当前 Book / 用户材料是否已经足够稳定到可以成文。只检查权威输入、未决高影响问题、当前章 Outline、人物执行态、关系/信息边界、连续性与项目自带 Style/Knowledge 合同是否已就绪；不规定句式、段落、文风或审美，不替代 Book Style，也不撰写正式正文。Not Ready 时只列阻塞项、责任 Owner 与建议；Ready 后正式成文继续按当前项目 / Storybook 合同执行。"
visibility: workflow
phase: verification
---
...
本 Skill 是前置 Gate，不是通用正文写作器，也不是文风规范。它只验证“输入是否够用”；正文具体写成什么样，继续由当前 Book / 项目的 Style、Characters、Story、Outline、Knowledge / Mistakes 等权威合同决定。
...
- “直接写第一章 / 下一章”——先执行本 Gate；Ready 后退出本 Skill，再按项目合同成文。
- “帮我检查写正文还缺什么。”

邻接边界：

- 用户要**新增、细化、拆合或修改章纲** → `writing/fiction-outline`；
...
3. 读取项目声明为正文前置输入的内容；Storybook 常见输入包括 Book README、相关 World / Characters / Story / Style、当前与相邻 Outline、上一章 Manuscript 结尾、活动 Issue / pending sync、Knowledge / Mistakes。
...
- 用户要求“检查后直接写”时，只有 `Ready` 才继续，且后续正文直接服从当前项目 / Book 合同，本 Skill 不提供额外通用文风规则。
...
- Gate 已 Ready → 本 Skill 完成；若用户还要求正文，退出本 Skill，按当前项目合同继续正式成文。
